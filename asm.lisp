  ...
  (defun emit-asm (&key code (str nil)  (level 0) (env nil))
    (flet ((emit (code &key (dl 0) env)
	  ...
	  (if (listp code)
		...
		(lea (destructuring-bind (dst src) (cdr code)
		 (format nil "lea ~a, ~a" (emit dst) (emit src))))
		(add (destructuring-bind (dst src) (cdr code)
		 (format nil "add ~a, ~a" (emit dst) (emit src))))
		(sub (destructuring-bind (dst src) (cdr code)
		 (format nil "sub ~a, ~a" (emit dst) (emit src))))
		(imul (destructuring-bind (dst src) (cdr code)
			(format nil "imul ~a, ~a" (emit dst) (emit src))))
		(or (destructuring-bind (dst src) (cdr code)
			(format nil "or ~a, ~a" (emit dst) (emit src))))
		(and (destructuring-bind (dst src) (cdr code)
		       (format nil "and ~a, ~a" (emit dst) (emit src))))
		(xor (destructuring-bind (dst src) (cdr code)
			(format nil "xor ~a, ~a" (emit dst) (emit src))))


		(inc (destructuring-bind (dst) (cdr code)
		       (format nil "inc ~a" (emit dst))))
		(dec (destructuring-bind (dst) (cdr code)
		       (format nil "dec ~a" (emit dst))))
		(not (destructuring-bind (dst) (cdr code)
		       (format nil "not ~a" (emit dst))))
		(data
